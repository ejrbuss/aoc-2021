...
type BingoBoardResult = { moves: number; score: number };

const WinningMasks: number[] = [
	// horizontal
...
function parseInput(input: string): {
	...
	const numbers = (lines.shift() as string)
		.trim()
		.split(",")
		.map((n) => parseInt(n)) as any;
	const boards = chunk(
	...
	return { boards, numbers: numbers as any };
}

function computeResult(board: BingoBoard, numbers: number[]): BingoBoardResult {
...
function check(marked: number): boolean {
