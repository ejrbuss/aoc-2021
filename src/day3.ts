...
type BinaryString = number[];

function parseInput(input: string): BinaryString[] {
	return parseLines(input).map((line) =>
		line.split("").map((digit) => parseInt(digit))
	);
}

function toNumber(binStr: BinaryString): number {
	return parseInt(binStr.join(""), 2);
}

function count(diagnostics: BinaryString[]): BinaryString {
	return diagnostics.reduce((counts, diagnostic) => {
		return zipWith(counts, diagnostic, (a, b) => a + b);
	});
}

function gamma(diagnostics: BinaryString[]): BinaryString {
	return count(diagnostics).map((count) => {
		return Math.round(count / diagnostics.length);
	});
}

function epsilon(diagnostics: BinaryString[]): BinaryString {
	return count(diagnostics).map((count) => {
